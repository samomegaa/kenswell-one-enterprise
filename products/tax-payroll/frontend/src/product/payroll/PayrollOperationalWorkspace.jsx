...
import StaffologyPayrollRunWorkspace from '../../workspaces/staffology/payroll-run/StaffologyPayrollRunWorkspace';
...
import { PAYROLL_WORKFLOW_STAGES, getPayrollWorkflowSummary } from './payrollWorkflow';
...
export default function PayrollOperationalWorkspace({ context }) {
  ...
  return (
    <section className="payroll-operational-workspace">
      ...
      <PayrollRuntimeCard session={sessionState.session} active={sessionState.active} onDeactivate={sessionState.deactivate} />
      <PayrollPeriodCard period={periodState.period} active={periodState.active} onClose={periodState.close} />
      <PayrollExecutionCard execution={orchestrator.execution} nextState={orchestrator.nextState} onAdvance={orchestrator.advance} onPause={orchestrator.pause} onResume={orchestrator.resume} onCancel={orchestrator.cancel} />
      <PayrollPipelineCard pipeline={pipelineState.pipeline} metrics={pipelineState.metrics} onAdvance={pipelineState.advance} onPause={pipelineState.pause} onResume={pipelineState.resume} onCancel={pipelineState.cancel} />
      <PayrollWorkflowRail stages={PAYROLL_WORKFLOW_STAGES} activeId={orchestrator.execution?.state || periodState.stage} />
      <div className="payroll-operational-workspace__content">
        {operational && sessionState.session?.runtimeWorkspace ? <StaffologyPayrollRunWorkspace runtimeWorkspace={sessionState.session.runtimeWorkspace} /> : <PayrollRuntimeRequired />}
      </div>
    </section>
  );
}
PayrollOperationalWorkspace.propTypes = { context: PropTypes.object.isRequired };
