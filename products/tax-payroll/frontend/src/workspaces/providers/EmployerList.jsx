...
export default function EmployerList({ employers, linkedClient, onLink, onOpenClientPayroll }) {
  if (!employers.length) {
    return <section className="provider-empty-state">No employers were returned by Staffology.</section>;
  }
  ...
  return (
    <div className="provider-employer-list">
      {employers.map((employer) => {
        const linked =
          linkedClient?.payroll?.employer?.provider?.externalEmployerId ===
          employer.externalEmployerId;

        return (
          <article
            className={linked ? 'provider-employer-row linked' : 'provider-employer-row'}
          ...
          >
            ...
            <div className="provider-employer-year">
              <span>Tax year</span>
              <strong>
                {employer.currentTaxYear || employer.startTaxYear || 'Not supplied'}
              </strong>
            </div>
            <button
              type="button"
            ...
            >
              {linked ? 'Linked' : 'Link to client'}
            </button>
            {linked && (
              <LinkedEmployerBridge
                client={linkedClient}
                onOpenPayroll={() => onOpenClientPayroll(linkedClient)}
              />
            )}
          </article>
        );
